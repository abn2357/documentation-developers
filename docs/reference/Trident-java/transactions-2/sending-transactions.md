...
`TransactionBuilder` allows you to add or modify parameters before signing, for example, setting feelimit, or adding memo information for the transaction:
...
> 📘 Note
>
> Transaction hash may change due to any modification to the original transaction (except the signature). The hash is re-calculated before broadcasting.
...
By default, Trident builds transactions locally but needs to query the fullnode for the latest block information:
...
* [Trident API Reference](/reference/trident-apis)
