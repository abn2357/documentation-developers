...
The parameter `permissionId` designates the permission type, with Owner ID = 0, Witness ID = 1, and the Active ID incremented from 2 onwards. When the contract is executed, the ID is used to specify which permission to use. For details on Java-Tron permissionId, please refer to the <a href="https://developers.tron.network/docs/multi-signature" target="_blank">Multi-Signature guide</a>. 
