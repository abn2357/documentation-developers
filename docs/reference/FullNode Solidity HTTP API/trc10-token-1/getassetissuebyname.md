...
excerpt: Query a token by name, returns token info. (Confirmed state)
api:
...
AssetIssueCapsule : Token found with the provided name, please refer to [GetAssetIssueById](/reference/getassetissuebyid-1) for detailed fields, please note that string fields are in HexString format.
